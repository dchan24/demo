...
public class MessageCache {
    ...
    private MessageCache.Status status = MessageCache.Status.PENDING;
    private MessageCache.OnLoadFinishListener mListener;
    ...
    public int prepareMessageCache(final Context context, final String address, final Bundle bundle) {
        final int pId = ++prepardId;
        LogF.i(TAG, "----prepareMessageCache------" + address + " pId = " + pId);

        status = MessageCache.Status.RUNNING;
        mListener = null;
        ...
        helper.runInThread(new Func1<Object, Bundle>() {
            ...
            public Bundle call(Object o) {
                ...
                String[] projection = new String[]{
                        "_id", Type.TYPE_BOX_MESSAGE + " AS box_type", "msg_id", "address", "send_address", "person", "body",
                        ...
                        BaseModel.COLUMN_EXACT_READ, BaseModel.COLUMN_EXD_SEND_STATUS, "show_send", "message_receipt"};
                Cursor cursor = context.getContentResolver().query(Conversations.Message.CONTENT_URI, projection, where, null, order);
            ...
            public Object call(Bundle extra) {
                if (pId == prepardId) {
                    LogF.i(TAG, "----prepareMessageCache over------" + address);

                    mExtra = extra;
                    status = Status.FINISHED;
                    mAddress = address;
                    if (mListener != null) {
                        getMessageCache(mListener, pId);
                        mListener = null;
                        mExtra = null;
    ...
    private void changeCursorToData(Cursor cursor, Bundle extra) {
        ...
        if (cursor.moveToFirst()) {
            do {
                Message value = (Message) MessageCursorLoader.getValueFromCursor(cursor, Message.class, false);
                cacheList.add(value);
    ...
    public void getMessageCache(MessageCache.OnLoadFinishListener listener, int pId) {
        LogF.i(TAG, "----getMessageCache------" + mAddress + " pId = " + pId);

        if (pId == prepardId) {
            if (status == MessageCache.Status.FINISHED) {
